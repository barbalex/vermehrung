...
export const herkunftModelBase = ModelBase.named('herkunft')
  .props({
    ...
    herkunft_files_aggregate: types.union(
      types.undefined,
      types.late(() => herkunft_file_aggregateModel),
    ),
    id: types.identifier,
    ...
    kulturs_aggregate: types.union(
      types.undefined,
      types.late(() => kultur_aggregateModel),
    ),
    ...
    nr: types.union(types.undefined, types.null, types.number, types.string),
    sammlungs: types.union(
      types.undefined,
      types.array(MSTGQLRef(types.late(() => sammlungModel))),
    ),
    sammlungs_aggregate: types.union(
      types.undefined,
      types.late(() => sammlung_aggregateModel),
    ),
...
export class herkunftModelSelector extends QueryBuilder {
  ...
  herkunft_files(builder) {
    return this.__child(`herkunft_files`, herkunft_fileModelSelector, builder)
  }
  herkunft_files_aggregate(builder) {
    return this.__child(
...
export const herkunftModelPrimitives = selectFromherkunft()._conflicts._depth
  ._parent_rev._rev._revisions.bemerkungen.changed.changed_by.gemeinde
  .geom_point.kanton.land.lokalname.lv95_x.lv95_y.nr.tsv.wgs84_lat.wgs84_long
