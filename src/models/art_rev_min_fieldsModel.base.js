/* This is a mst-gql generated file, don't modify it manually */
/* eslint-disable */

import { types } from "mobx-state-tree"
import { QueryBuilder } from "mst-gql"
import { ModelBase } from "./ModelBase"


/**
 * art_rev_min_fieldsBase
 * auto generated base class for the model art_rev_min_fieldsModel.
 */
export const art_rev_min_fieldsModelBase = ModelBase
  .named('art_rev_min_fields')
  .props({
    __typename: types.optional(types.literal("art_rev_min_fields"), "art_rev_min_fields"),
    _depth: types.union(types.undefined, types.null, types.integer),
    _parent_rev: types.union(types.undefined, types.null, types.string),
    _rev: types.union(types.undefined, types.null, types.string),
    _rev_at: types.union(types.undefined, types.null, types.frozen()),
    ae_id: types.union(types.undefined, types.null, types.frozen()),
    art_id: types.union(types.undefined, types.null, types.frozen()),
    changed: types.union(types.undefined, types.null, types.frozen()),
    changed_by: types.union(types.undefined, types.null, types.string),
    id: types.identifier,
  })
  .views(self => ({
    get store() {
      return self.__getStore()
    }
  }))

export class art_rev_min_fieldsModelSelector extends QueryBuilder {
  get _depth() { return this.__attr(`_depth`) }
  get _parent_rev() { return this.__attr(`_parent_rev`) }
  get _rev() { return this.__attr(`_rev`) }
  get _rev_at() { return this.__attr(`_rev_at`) }
  get ae_id() { return this.__attr(`ae_id`) }
  get art_id() { return this.__attr(`art_id`) }
  get changed() { return this.__attr(`changed`) }
  get changed_by() { return this.__attr(`changed_by`) }
  get id() { return this.__attr(`id`) }
}
export function selectFromart_rev_min_fields() {
  return new art_rev_min_fieldsModelSelector()
}

export const art_rev_min_fieldsModelPrimitives = selectFromart_rev_min_fields()._depth._parent_rev._rev._rev_at.ae_id.art_id.changed.changed_by
