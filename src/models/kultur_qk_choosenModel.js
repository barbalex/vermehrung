import { kultur_qk_choosenModelBase } from "./kultur_qk_choosenModel.base"


/* A graphql query fragment builders for kultur_qk_choosenModel */
export { selectFromkultur_qk_choosen, kultur_qk_choosenModelPrimitives, kultur_qk_choosenModelSelector } from "./kultur_qk_choosenModel.base"

/**
 * kultur_qk_choosenModel
 *
 * columns and relationships of "kultur_qk_choosen"
 */
export const kultur_qk_choosenModel = kultur_qk_choosenModelBase
  .actions(self => ({
    // This is an auto-generated example action.
    log() {
      console.log(JSON.stringify(self))
    }
  }))
