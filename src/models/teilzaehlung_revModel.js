import { teilzaehlung_revModelBase } from "./teilzaehlung_revModel.base"


/* A graphql query fragment builders for teilzaehlung_revModel */
export { selectFromteilzaehlung_rev, teilzaehlung_revModelPrimitives, teilzaehlung_revModelSelector } from "./teilzaehlung_revModel.base"

/**
 * teilzaehlung_revModel
 *
 * columns and relationships of "teilzaehlung_rev"
 */
export const teilzaehlung_revModel = teilzaehlung_revModelBase
  .actions(self => ({
    // This is an auto-generated example action.
    log() {
      console.log(JSON.stringify(self))
    }
  }))
